...
class lab_3 {
    static public void main(String[] args) throws InappropriateFunctionPointException {
        ...
        for (int i = 0; i<5;i++){
            System.out.printf("f%d = (%.1f, %.3f)%n", i, LLTF1.getPointX(i),LLTF1.getPointY(i));
        }
    ...
    static void TestTabulatedFunction(TabulatedFunction function) throws InappropriateFunctionPointException {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            System.out.printf("f%d: (%.1f; %.3f)%n", i + 1, function.getPointX(i), function.getPointY(i));
        }
        ...
        for (double x : test) {
            if (Double.isNaN(function.getFunctionValue(x))) {
                ...
                System.out.printf("f(%.1f) = %.3f%n", x, function.getFunctionValue(x));
            }
        }
        ...
        System.out.printf("Точка f5 = (%.1f, %.3f)%n", function.getPointX(4), function.getPointY(4));
        ...
        System.out.printf("Исходная точка: f3'= (%.1f; %.3f)%n", function.getPointX(2), function.getPointY(2));
        function.setPoint(2, test_point);
        System.out.printf("Измененная точка: f3'= (%.1f; %.3f)%n", function.getPointX(2), function.getPointY(2));
        System.out.println("Точки функции:");
        for (int i = 0; i < function.getPointsCount(); i++) {
            System.out.printf("f%d: (%.1f; %.3f)%n", i + 1, function.getPointX(i), function.getPointY(i));
        }
        ...
        System.out.printf("Исходная точка: f6'= (%.1f; %.3f)%n", function.getPointX(5), function.getPointY(5));
        function.setPointX(5, 9.5);
        function.setPointY(5, Math.sqrt(9.5));
        System.out.printf("Измененная точка: f6'= (%.1f; %.3f)%n", function.getPointX(5), function.getPointY(5));
        System.out.println("Точки функции:");
        for (int i = 0; i < function.getPointsCount(); i++) {
            System.out.printf("f%d: (%.1f; %.3f)%n", i + 1, function.getPointX(i), function.getPointY(i));
        }
        ...
        try{
            ...
            System.out.printf("       Попытка добавить точку F (%.1f, %.3f):%n", function.getPointX(1),function.getPointY(1));
